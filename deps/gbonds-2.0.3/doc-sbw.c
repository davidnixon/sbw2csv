...
/****************************************************************************/
gbDoc *gb_doc_sbw_open(const gchar *filename, gbStatus *status) {
  FILE *fp;
  gchar line[256];
  gint version = 0;
  gbDoc *doc = NULL;
...
static gbDoc *read_sbw4(FILE *fp, gbStatus *status) {
  ...
  *status = GB_OK;
  ...
  for (i = 0; i < n; i++) {
    ...
    if ((strcasecmp("E", series) == 0) || (strcasecmp("S", series) == 0) ||
        (strcasecmp("EE", series) == 0) || (strcasecmp("I", series) == 0)) {

      idate_string = gb_date_fmt(info_fixed.idate + SBW4_EPOCH);
      p_bond =
          gb_doc_bond_new(series, idate_string, info_fixed.denom, sn, status);
      g_free(idate_string);
      if (*status != GB_OK) {
        g_warning(("Cannot create bond, status = %d"), *status);
        g_object_unref(G_OBJECT(doc));
      ...
      *status = gb_doc_add_bond(doc, p_bond);
      ...
      g_message(("Skipping unsupported bond series (%s)"), series);
    }
  }
