	...
	if (p_bond->sn != NULL)
	{
		...
		for (li = doc->list; li != NULL; li = li->next)
		{
			...
			if (p_bond_li->sn != NULL)
			{
				...
				{
					printf(("Duplicate SN = %s"), p_bond->sn);
				...
				gbStatus *status)
{
	...
	*status = gb_rules_determine_issue(p_bond->series, p_bond->denom, &p_bond->issue);
	if (*status != GB_OK)
	{
		printf(("Cannot determine issue price"));
		gb_doc_bond_free(p_bond);
		p_bond = NULL;
		return NULL;
	}
	...
	*status = gb_rules_determine_maturity(p_bond->series, p_bond->idate,
	...
	if (*status != GB_OK)
	{
		printf(("Cannot determine maturity data"));
		gb_doc_bond_free(p_bond);
		p_bond = NULL;
		return NULL;
	}
					 ...
					 gbStatus *status)
{
	...
	if (p_info == NULL)
	{
		printf(("Cannot allocate bond information structure!"));
		*status = GB_ERROR_MEM_ALLOC;
		return NULL;
	}
	...
	*status = gb_date_parse(rdate, &sb_rdate);
	if (*status != GB_OK)
	{
		printf(("Cannot parse redemption date"));
	...
	*status = gb_rules_get_last_accrual(p_bond->series,
	...
	if (*status != GB_OK)
	{
		printf(("Cannot determine last accrual date"));
		gb_doc_bond_free_info(p_info);
		return NULL;
	}
	*status = gb_rules_get_next_accrual(p_bond->series,
	...
	if (*status != GB_OK)
	{
		printf(("Cannot determine next accrual date"));
		gb_doc_bond_free_info(p_info);
		return NULL;
	}
