...
enum { CHANGED, NAME_CHANGED, MODIFIED_CHANGED, LAST_SIGNAL };
...
static gint insert_compare_fct(const gbDocBond *a, const gbDocBond *b);
...
GType gb_doc_get_type(void) {
  ...
  if (!type) {
    ...
    type = g_type_register_static(G_TYPE_OBJECT, "gbDoc", &info, 0);
  }
...
static void gb_doc_class_init(gbDocClass *klass) {
  ...
  signals[CHANGED] =
      g_signal_new("changed", G_OBJECT_CLASS_TYPE(object_class),
                   G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(gbDocClass, changed),
                   NULL, NULL, gb_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[NAME_CHANGED] =
      g_signal_new("name_changed", G_OBJECT_CLASS_TYPE(object_class),
                   G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(gbDocClass, name_changed),
                   NULL, NULL, gb_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[MODIFIED_CHANGED] = g_signal_new(
      "modified_changed", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(gbDocClass, modified_changed), NULL, NULL,
      gb_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
...
/* Is doc modified?                                                         */
/****************************************************************************/
gboolean gb_doc_is_modified(gbDoc *doc) { return doc->_private->modified_flag; }
...
/* Can undo?                                                                */
/****************************************************************************/
gboolean gb_doc_can_undo(gbDoc *doc) { return FALSE; }

/****************************************************************************/
/* Can redo?                                                                */
/****************************************************************************/
gboolean gb_doc_can_redo(gbDoc *doc) { return FALSE; }
...
gbStatus gb_doc_add_bond(gbDoc *doc, gbDocBond *p_bond) {
    ...
    for (li = doc->list; li != NULL; li = li->next) {
      ...
      if (p_bond_li->sn != NULL) {
        if (strcmp(p_bond->sn, p_bond_li->sn) == 0) {
          g_warning(("Duplicate SN = %s"), p_bond->sn);
  ...
  doc->list =
      g_list_insert_sorted(doc->list, p_bond, (GCompareFunc)insert_compare_fct);
...
/* Get inventory title.                                                     */
/****************************************************************************/
gchar *gb_doc_get_title(gbDoc *doc) { return g_strdup(doc->_private->title); }
...
gbDocBond *gb_doc_bond_new(gchar *series, const gchar *idate, gdouble denom,
  ...
  if (p_bond == NULL) {
    g_warning(("Cannot allocate bond!"));
  ...
  *status = gb_date_parse(idate, &p_bond->idate);
  if (*status != GB_OK) {
    printf(("Cannot parse issue date"));
    gb_doc_bond_free(p_bond);
    p_bond = NULL;
    return NULL;
  }
  ...
  *status = gb_rules_test_issue(p_bond->series, p_bond->idate);
  ...
  /*-----------------------*/
  *status =
      gb_rules_determine_issue(p_bond->series, p_bond->denom, &p_bond->issue);
  if (*status != GB_OK) {
    g_warning(("Cannot determine issue price"));
    gb_doc_bond_free(p_bond);
    p_bond = NULL;
    return NULL;
  }
  ...
  *status = gb_rules_determine_maturity(p_bond->series, p_bond->idate,
                                        &p_bond->mdate);
  if (*status != GB_OK) {
    g_warning(("Cannot determine maturity data"));
    gb_doc_bond_free(p_bond);
    p_bond = NULL;
    return NULL;
  }
...
gbDocBond *gb_doc_bond_new_from_sn_idate(gchar *sn, const gchar *idate,
  ...
  *status = gb_date_parse(idate, &p_bond->idate);
  if (*status != GB_OK) {
    g_warning(("Cannot parse issue date"));
    gb_doc_bond_free(p_bond);
    return NULL;
  }
...
gbDocBondInfo *gb_doc_bond_get_info(gbDocBond *p_bond, const gchar *rdate,
  ...
  if (p_info == NULL) {
    g_warning(("Cannot allocate bond information structure!"));
    *status = GB_ERROR_MEM_ALLOC;
    return NULL;
  }
  ...
  *status = gb_date_parse(rdate, &sb_rdate);
  if (*status != GB_OK) {
    g_warning(("Cannot parse redemption date"));
  ...
  /*--------------------------------------------*/
  p_info->value = gb_table_model_get_value(
      table_model, p_bond->series, p_bond->denom, p_bond->issue, p_bond->idate,
      sb_rdate, &p_info->nopay_flag);
  if (*status != GB_OK) {
    g_warning(("Cannot determine value"));
    gb_doc_bond_free_info(p_info);
    return NULL;
  }
  p_info->value_str = gb_value_fmt(p_info->value, TRUE);
  p_info->interest = p_info->value - p_info->issue_price;
  *status = gb_rules_get_last_accrual(p_bond->series, p_bond->idate, sb_rdate,
                                      p_bond->mdate, &sb_ladate);
  if (*status != GB_OK) {
    g_warning(("Cannot determine last accrual date"));
    gb_doc_bond_free_info(p_info);
    return NULL;
  }
  *status = gb_rules_get_next_accrual(p_bond->series, p_bond->idate, sb_rdate,
                                      p_bond->mdate, &sb_nadate);
  if (*status != GB_OK) {
    g_warning(("Cannot determine next accrual date"));
    gb_doc_bond_free_info(p_info);
    return NULL;
  }
  *status = gb_determine_yield(p_info->issue_price, p_info->value,
                               p_bond->idate, sb_ladate, &p_info->yield);
  if (*status != GB_OK) {
    g_warning(("Cannot determine yield"));
    gb_doc_bond_free_info(p_info);
    return NULL;
  }
  ...
  *status = gb_rules_determine_exchangeability(p_bond->series, p_bond->idate,
  ...
  p_info->flag_str = g_strdup_printf("%s%s", (p_info->matured_flag ? "M" : ""),
                                     (p_info->nopay_flag ? "*" : ""));
...
/****************************************************************************/
void gb_doc_bond_free_info(gbDocBondInfo *p_info) {
  g_free(p_info->sn);
  p_info->sn = NULL;
