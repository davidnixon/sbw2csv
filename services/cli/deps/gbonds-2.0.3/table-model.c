...
static GList *append_tables_from_dir(GList *table, const gchar *dirname);

static GList *append_table_from_file(GList *table, const gchar *fname);

static GList *append_entry_from_line(GList *table, const gchar *line);

static void free_table(GList *table);

static gint extract_int(const gchar *ptr, gint digits);
...
GType gb_table_model_get_type(void) {
  ...
  if (!type) {
    ...
    type = g_type_register_static(G_TYPE_OBJECT, "gbTableModel", &info, 0);
  }
...
static void gb_table_model_class_init(gbTableModelClass *klass) {
  ...
  signals[CHANGED] = g_signal_new(
      "changed", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(gbTableModelClass, changed), NULL, NULL,
      gb_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void gb_table_model_instance_init(gbTableModel *table_model) {
...
static GList *append_entry_from_line(GList *table, const gchar *line) {
  ...
  if (p_entry == NULL) {
    g_warning(("Cannot allocate redemption table entry."));
    return table;
  }
  ...
  default:
    g_free(p_entry);
    p_entry = NULL;
    g_warning(("Problem parsing redemption table entry, unknown series."));
    return table;
  }
  ...
  if (!p_entry->r_year || !p_entry->r_month || !p_entry->i_year) {
    g_free(p_entry);
    p_entry = NULL;
    g_warning(("Problem parsing redemption table entry, bad date strings."));
    return table;
  }
  ...
  for (i = 0; i < 12; i++) {
    if (i_char < n_char) {
      p_entry->r_value[i] = ((gdouble)extract_int(&line[i_char], 6)) / 100.0;
    } else {
...
/* value of bond.                                                            */
/*****************************************************************************/
gdouble gb_table_model_get_value(gbTableModel *table_model, gbSeries series,
                                 gdouble denom, gdouble issue_price,
                                 gbDate idate, gbDate rdate,
                                 gboolean *nopay_flag) {
  GList *p;
  RedemptionEntry *p_entry;
  gdouble rvalue;
  ...
  g_warning(("No matching redemption value"));
  *nopay_flag = TRUE;
  return 0.0;
}
